...
- `debug/index-v1.json`：调试通道。读取手动 `debug-<完整提交 SHA>` 标签对应的预发布；普通分支推送不会发布。
...
插件仓库名称需要以 `plugin-` 开头。日常 CI 只测试并上传构建产物。维护者手动推送 `debug-<完整提交 SHA>` 标签后，Debug 发布工作流会：

1. 完成测试并构建调试 `.atsplugin`；
2. 校验标签与构建提交一致，上传完整资产后公开该标签的不可变预发布；
3. 向本仓库发送更新事件。
